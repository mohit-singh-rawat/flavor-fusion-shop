...
import { getTrendingProductsAction } from '../redux/recommendations/action';
...
const TrendingProducts = () => {
  ...
  const trendingProductsState = useSelector((state) => state.trendingProducts);
  ...
  if (trendingProductsState.loading) {
    return (
      <div className="mt-8">
  ...
  console.log('Trending Products State:', trendingProductsState);
  
  if (!trendingProductsState.data || trendingProductsState.data.length === 0) {
    return (
      <div className="mt-8">
        <div className="flex items-center mb-6">
          <TrendingUp className="w-6 h-6 mr-2 text-orange-600" />
  ...
  return (
    <div className="mt-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {trendingProductsState.data.slice(0, 4).map((product, index) => (
          <Card key={product._id} className="group hover:shadow-lg transition-shadow relative">
            {index === 0 && (
